...
# save_path = None
x = train['Text'].tolist()[:1000]
y = train['Cat1'].tolist()[:1000]
...
t2g = Text2GraphTransformer(n_jobs=8, min_df=5, save_path=save_path, verbose=1, max_df=0.1)
...
print(g.x.shape)
test = g.x.to_dense()
test = test[len(x):, len(x):]
print("Graph built")

print(test.shape)

gcn = TextGAT(g.n_vocab, len(np.unique(y)), 1, test)

cv = t2g.cv
x = cv.transform(x).todense()
...
train_data = CustomData(th.FloatTensor(x), th.LongTensor(y))
...
for epoch in range(epochs):
    for i_batch, sample_batched in enumerate(data_loader):
        ...
        gcn.eval()
        with th.no_grad():
            pred_val = np.argmax(gcn(x_test).cpu().numpy(), axis=1)
            pred_train = np.argmax(gcn(x).cpu().numpy(), axis=1)
